import { MatrixSettings } from '../types/matrix';
import { renderMatrixRain } from '../effects/matrixRainRenderer';
...
export const renderImage = (params: RenderImageParams) => {
  const { image: loadedImage, effect: activeEffect, settings, sourceCanvas, outputCanvas } = params;
  
  // Destructure settings
  const {
    invert, brightness, contrast, gamma, blur,
    ...
    colorMode, foreground, background,
    halftoneShape, halftoneSpacing, halftoneDotScale,
    contourDensity, contourLevels,
    dotsSpacing, dotsSize,
    voronoiSiteCount, voronoiCellPadding, voronoiStyle,
    crosshatchAngle, crosshatchSpacing, crosshatchThickness,
    noiseEnabled, noiseAmount, noiseMonochrome, noiseScale, noiseSeed,
    waveEnabled, waveAmplitude, waveFrequency, waveSpeed,
    pixelSortEnabled, pixelSortThreshold, pixelSortAngle,
    matrixSettings,
    setIsProcessing, setAsciiText, setGridData,
    phosphorEnabled, crtCurveEnabled, scanlinesEnabled, vignetteEnabled,
    selectedPaletteIndex
,
    halftoneAngle, halftoneBackground, voronoiContrast, waveAnimate, crosshatchRandomness, asciiBgColor, hexToRgb, asciiCharSet, crosshatchDensity, vhsContrast, outputCanvasRef, waveAmplitude, dotsGridType, halftoneDotScale, noiseType, thresholdPoint, voronoiCellSize, halftoneContrast, crosshatchContrast, crosshatchInvert, sortMode, sortContrast, edgeContrast, compareMode, edgeBgColor, BAYER_8X8, asciiContrast, blockifyStyle, waveLineThickness, crosshatchLineColor, dotsShape, BAYER_4X4, crosshatchAngle, halftoneForeground, blockifyContrast, vhsBrightness, thresholdLevels, voronoiEdgeWidth, dotsBrightness, modulation, waveFrequency, blockifyBorderWidth, brightness, edgeInvert, halftoneInvert, thresholdBackground, selectedPaletteIndex, sortRandomness, contourBrightness, crosshatchBgColor, edgeBrightness, asciiColorMode, crosshatchBrightness, waveDirection, waveLineCount, sortStreakLength, edgeColorMode, edgeLineWidth, activeEffect, halftoneColorMode, noiseOctaves, voronoiBrightness, blockifySize, vhsColorBleed, waveColorMode, applyEdgeDetectionPipeline, asciiBrightness, noiseDistortOnly, invert, dotsSize, voronoiRandomize, chromaticEnabled, sourceCanvasRef, crosshatchLineWidth, setIsProcessing, thresholdContrast, waveContrast, waveBgColor, contourColorMode, waveBrightness, RETRO_PALETTES, sortReverse, noiseIntensity, contourFillMode, voronoiEdgeColor, loadedImage, noiseSpeed, matrixSettings, edgeColor, noiseBrightness, colorMode, sortIntensity, edgeThreshold, sortDirection, noiseAnimate, blockifyBorderColor, dotsColorMode, vhsTrackingError, contourContrast, asciiScale, noiseContrast, halftoneShape, sortBrightness, foreground, vhsNoise, edgeAlgorithm, blockifyColorMode, asciiGamma, thresholdDither, thresholdForeground, BAYER_2X2, contourLevels, contourInvert, halftoneBrightness, algorithm, dotsContrast, maxDisplace, halftoneSpacing, thresholdColorMode, vhsScanlines, gamma, crosshatchLayers, getNoiseValue, vhsDistortion, thresholdBrightness, dotsInvert, background, noiseScale, blockifyBrightness, contrast, waveLineColor, asciiSpacing, sortThreshold, asciiCharGridRef, renderMatrixRain, dotsSpacing, thresholdInvert, applyWaveLinesPipeline, voronoiColorMode
  } = settings;

  const sourceCanvasRef = { current: sourceCanvas };
  const outputCanvasRef = { current: outputCanvas };
